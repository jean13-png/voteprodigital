...
export default function CandidatForm({ mode, defaultValues = {} }: CandidatFormProps) {
  ...
  function handleProjectImageSelection(e: React.ChangeEvent<HTMLInputElement>) {
    const file = e.target.files?.[0];
    if (!file) return;
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center gap-3">
                <label className="inline-flex cursor-pointer items-center gap-2 rounded-lg border border-[#1B2A6B]/30 bg-white px-3 py-2 text-xs font-medium text-[#1B2A6B] hover:bg-[#1B2A6B]/5">
                  ...
                  Uploader une image
                  <input type="file" accept="image/jpeg,image/png,image/webp" className="hidden" onChange={handleProjectImageSelection} />
                </label>
                {projectImagePreview && (
